...
export default function TutorDashboard() {
  ...
  useEffect(() => {
    ...
    if (parsed.tutorProfile) {
      ...
      setKycSubmitted(
        !!parsed.tutorProfile.phone &&
          parsed.tutorProfile.kycStatus !== "REJECTED",
      );
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const tutorProfileId =
        data?.user?.tutorProfile?.id || user?.tutorProfile?.id;
      if (tutorProfileId) {
        const availRes = await fetch(
          `http://localhost:4000/api/availability/${tutorProfileId}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (availRes.ok) {
  ...
  const deleteKycDocument = async () => {
    if (
      !confirm(
        "Are you sure you want to delete your KYC document? Your verification status may be affected.",
      )
    )
      return;
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(
        "http://localhost:4000/api/profile/kyc-document",
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const deleteAccount = async () => {
    if (
      !confirm(
        "⚠️ Are you sure you want to permanently delete your account? This action cannot be undone. All your data, bookings, and messages will be lost.",
      )
    )
      return;
    if (
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-4xl">
      <div className="mb-6">
        ...
        <p className="text-[13px] text-gray-500 mt-0.5">
          Manage your classes and profile.
        </p>
      </div>
      ...
      {!kycSubmitted && !isVerified ? (
            ...
            <div className="mb-6 bg-rose-50 border border-rose-100 rounded-xl p-4 flex items-start gap-3 text-rose-800 animate-in fade-in">
              ...
              <div>
                ...
                <p className="text-[11px] text-rose-600/90 mt-0.5">
                  ...
                  team. Please update your details and resubmit.
                </p>
              </div>
            </div>
          )}
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-500 font-medium">
                Submit KYC details to start earning.
              </p>
            </div>
          </div>
          <form onSubmit={submitKYC} className="space-y-4 max-w-sm">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-gray-50 rounded-xl border border-transparent focus:bg-white focus:border-orange-200 focus:ring-2 focus:ring-orange-50 outline-none text-[13px] transition-all"
              />
              <p className="text-[10px] text-gray-400 mt-1">
                Add all subjects you can teach, separated by commas
              </p>
            </div>
            <div>
              <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
            ...
            <div>
              <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                KYC Document (ID Proof - PDF Only)
              </label>
              <input
                type="file"
              ...
              />
              <p className="text-[10px] text-gray-400 mt-1">
                Upload Aadhaar, PAN, or any government-issued ID in PDF format
              </p>
            </div>
            <button
              type="submit"
              disabled={isSubmittingKyc}
              className="w-full bg-gray-900 text-white py-3 rounded-xl text-[13px] font-bold hover:bg-black transition shadow-sm mt-2 flex justify-center items-center gap-2 disabled:opacity-50"
            >
        ...
        <div className="bg-gradient-to-br from-amber-50 to-white rounded-2xl p-6 md:p-8 border border-amber-100 shadow-sm flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-[13px] text-gray-600 mt-1 max-w-xl leading-relaxed">
              ...
              review by our admin team. You will be able to accept students once
              verified.
            </p>
            <button
              onClick={fetchData}
              className="mt-4 bg-white border border-amber-200 text-amber-700 px-4 py-2 rounded-lg text-[12px] font-bold shadow-sm hover:bg-amber-50 transition"
            >
              Check Status
            </button>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-emerald-50 to-white rounded-2xl p-5 md:p-6 border border-emerald-100 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-600 font-medium">
                You are visible to students and can receive bookings.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <StatCard
              title="Earnings"
              value={`₹${bookings.filter((b) => b.status === "COMPLETED").reduce((acc, b) => acc + b.amount, 0)}`}
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {(user.tutorProfile?.subjects || []).length > 0 ? (
                    user.tutorProfile.subjects.map((s: string, i: number) => (
                      <span
                        key={i}
                        className="bg-orange-50 text-[#F26522] px-2.5 py-1 rounded-lg text-[11px] font-bold border border-orange-100"
                      >
                        {s}
                      </span>
                    ))
            ...
            {user.tutorProfile?.kycDocument && (
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  <a
                    href={user.tutorProfile.kycDocument}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[12px] font-bold text-indigo-600 hover:underline flex items-center gap-1.5"
                  >
                    <FileText size={14} /> View Document
                  </a>
                  <button
                    onClick={() => forceDownload(user.tutorProfile.kycDocument)}
                    className="text-[12px] font-bold text-emerald-600 hover:underline flex items-center gap-1.5 transition"
                  >
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            ...
            {bookings.length === 0 ? (
              <div className="text-center py-6 text-gray-400 text-[12px] font-medium">
                ...
                {bookingsTotalPages > 1 && (
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                    <button
                      ...
                      className="px-3 py-1.5 text-[11px] font-bold text-gray-500 hover:text-[#F26522] disabled:opacity-50 disabled:hover:text-gray-500 bg-white border border-gray-200 rounded-lg shadow-sm transition-all"
                    >
                      Previous
                    </button>
        ...
        {!isVerified ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-gray-50/50">
            ...
            <p className="text-[13px] text-gray-500 mt-1 max-w-xs leading-relaxed">
              Admin verification required to message students.
            </p>
          </div>
        ) : (
                      ...
                      >
                        ...
                        >
                          {msg.message.startsWith("[FILE] ")
                            ...
                            : msg.message}
>>>>>>> 069f6dd66bc0237f1c372ae5895b4a79b43d760f
                        </div>
                      </div>
                    ) : chatError ? (
                ...
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-gray-100">
                  <div className="relative group cursor-pointer w-16 h-16">
                    {user.avatar ? (
                      ...
                      <div className="w-16 h-16 rounded-full bg-gradient-to-tr from-[#F26522] to-orange-100 flex items-center justify-center text-white font-bold text-2xl border border-orange-200/50 shadow-sm">
                        {user.name.charAt(0).toUpperCase()}
                      </div>
                    )}
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-transparent text-[13px] font-medium text-gray-800 focus:outline-none focus:bg-white focus:border-indigo-200 focus:ring-2 focus:ring-indigo-50 transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">
                      KYC Document Upload (PDF Only)
                    </label>
                    <div className="flex items-center gap-3">
                      <input
                        type="file"
                      ...
                      {kycDocumentUrl && (
                        <div className="flex items-center gap-2">
                          <a
                            href={kycDocumentUrl}
                            target="_blank"
                            className="text-indigo-600 text-[12px] font-bold hover:underline whitespace-nowrap"
                          >
                            View Current
                          </a>
                          <button
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    <label className="block text-[12px] font-bold text-gray-900 mb-3">
                      Weekly Availability
                    </label>
                    <div className="space-y-3">
                      {[
                        ...
                        return (
                          <div
                            key={day}
                            className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 bg-gray-50 rounded-xl"
                          >
